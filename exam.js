...
let militaryUnit = {
  ...
  function Mission1(militaryUnit) {
    return [militaryUnit.commandStructure.chiefOfStaff.rank, militaryUnit.commandStructure.chiefOfStaff.name, militaryUnit.commandStructure.chiefOfStaff.contact.phone].toString();
}
...
function Mission3(NewDeployment, militaryUnit) {
    ...
    let oldMission = militaryUnit.currentDeployment;
    let oldMissionReormatedToHistory = {
        
        eventDate: oldMission.startDate,
        
        eventDescription: oldMission.mission + " " + oldMission.location
        
    }
    militaryUnit.history.push(oldMissionReormatedToHistory);
    militaryUnit.currentDeployment = NewDeployment;
    console.log(militaryUnit.currentDeployment);
    return militaryUnit;
}
function Mission4(newWeapon, militaryUnit) {
    for (let i = 0; i < militaryUnit.equipment.firearms.length; i++) {
        if (militaryUnit.equipment.firearms[i].type === newWeapon.type) {
            militaryUnit.equipment.firearms[i].quantity++;
...
function Mission5(militaryUnit) {
    ...
    for (let i = 0; i < militaryUnit.trainingPrograms.length; i++) {
        sumWeaks += militaryUnit.trainingPrograms[i].duration;
    }
    return sumWeaks;
}
